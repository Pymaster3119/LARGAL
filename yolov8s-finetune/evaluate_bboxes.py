...
rf_regressor = None
rf_path = Path("reg_models/classifiers/hist_gb_model.pkl")
if not rf_path.exists():
    raise FileNotFoundError(f"Regressor file not found: {rf_path}")
...
try:
    ...
    # 2) try joblib (common for sklearn)
    try:
        joblib = importlib.import_module('joblib')
        try:
            rf_regressor = joblib.load(rf_path)
        except Exception:
            # joblib sometimes uses compressed files; try gzip+pickle
            with gzip.open(rf_path, 'rb') as gf:
                rf_regressor = pickle.load(gf)
    except Exception:
        # 3) try torch.load (if object was saved with torch)
        try:
            import torch
            rf_regressor = torch.load(str(rf_path), map_location='cpu')
        except Exception as e:
            raise RuntimeError(f"Failed to load regressor from {rf_path!s}: {e}")

# If the loaded object is a container (dict/list/tuple) try to find an inner object
# that implements .predict(X). This handles cases where joblib/torch saved a dict
# with metadata plus the estimator.
...
resolved_regressor = _find_predictor(rf_regressor)
if resolved_regressor is None:
    # keep original but warn
    print(f"Warning: loaded regressor is {type(rf_regressor)}, no .predict() found. You may need to edit the loader.")
    predictor = rf_regressor
else:
    predictor = resolved_regressor

# Compatibility guard: some sklearn versions change internal attributes when pickling/unpickling.
# If the loaded estimator is a HistGradientBoostingClassifier missing the internal
# `_preprocessor` attribute (common when loading across sklearn versions), create it
# as None so `.predict()` preprocessing check doesn't raise AttributeError.
try:
    import sklearn
    from sklearn.ensemble import HistGradientBoostingClassifier
    if isinstance(predictor, HistGradientBoostingClassifier) and not hasattr(predictor, '_preprocessor'):
        setattr(predictor, '_preprocessor', None)
except Exception:
    # If sklearn not available or import path differs, silently continue; we'll handle errors at predict time.
    pass
    ...
    for img_path in tqdm.tqdm(imgs):
        ...
        if hasattr(r, 'boxes') and r.boxes is not None:
            ...
            for i in range(len(xyxy)):
                ...
                class_pred = predictor.predict_proba(mu_in)
                confidence = np.max(class_pred)
                class_pred = np.argmax(class_pred, axis=1)
                class_pred = int(np.round(class_pred[0]))
...
if __name__ == "__main__":
    evaluate_bboxes(conf_threshold=0.65, conf = 0.001)
